...
def train_one_epoch(
    ...
    for batch_idx, (sequences, y_true, att_mask) in enumerate(dataloader):
        ...
        if config.arch.lower() == "convnova":
            # Use the attention mask so only real nucleotides are eligible
            # for masking. This excludes padded N's while allowing genuine
            # biological N's within the sequence.
        ...
        if config.arch.lower() == "convnova":
            # ConvNova paper: replace masked bases with N
        ...
        elif config.arch.lower() == "convnova":

            # NucleotideTokenizer already returns:
            # A=0 C=1 G=2 T=3 N=4
        ...
        if config.arch.lower() == "transformer":
            loss = criterion(
                ...
                # ConvNova predicts nucleotides (A,C,G,T,N)
                loss = criterion(
                    out.logits.view(-1, 5)[input_maskout.view(-1)],
                    targets.view(-1)[input_maskout.view(-1)],
                )
