...
def load_pretrained_model(checkpoint_path, device=None):
    ...
    if "config" not in ckpt:
        raise ValueError("Checkpoint does not contain training config.")

    config = ckpt["config"]
    arch = getattr(config, "arch", "transformer").lower()

    if arch == "transformer":

        assert "bert_config" in ckpt,             "Transformer checkpoint missing bert_config."
        ...
        from .convnova_long_wrapper import ConvNovaLongWrapper
