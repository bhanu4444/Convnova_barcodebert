...
class _SimpleVocab:
    def __init__(self, ordered_dict):
        self._itos = list(ordered_dict.keys())
    ...
    def set_default_index(self, idx): self._default_index = max(0, idx)

def build_vocab_from_dict(ordered_dict, **kwargs):
    return _SimpleVocab(ordered_dict)
...
class NucleotideTokenizer(object):
    """
    Converts DNA sequences directly into nucleotide IDs.
    ...
    def __init__(self, padding=False, max_len=660):
        self.padding = padding
        self.max_len = max_len
    ...
    def __call__(self, dna_sequence, offset=0):
        ...
        tokens = [self.mapping.get(base, 4) for base in x]

        att_mask = [1 if i < len(dna_sequence[offset:]) else 0
                    for i in range(self.max_len)]

        return (
            torch.tensor(tokens, dtype=torch.long),
            torch.tensor(att_mask, dtype=torch.int32),
        )
...
class DNADataset(Dataset):
    def __init__(
        ...
        elif tokenizer == "nucleotide":
            # Nucleotide tokenizer has no special tokens
            self.special_tokens = []
    ...
    with torch.no_grad():
        ...
        for barcode in df["nucleotides"]:
            ...
            if hasattr(model, "encode"):
                hidden = model.encode(x, att_mask)
            else:
                hidden = model(x, att_mask).hidden_states[-1]

            # ---------------------------------------------------------
            # Masked mean pooling
            # ---------------------------------------------------------
            sum_embeddings = (hidden * att_mask.unsqueeze(-1)).sum(dim=1)

            sum_mask = att_mask.sum(dim=1, keepdim=True)
