...
from barcodebert.model_factory import build_model
...
def setup_logger(log_path):
    ...
    LOG_FILE.write("\n")
    LOG_FILE.write("=" * 80 + "\n")
    LOG_FILE.write(f"Training session started: {datetime.now()}\n")
    LOG_FILE.write("=" * 80 + "\n\n")
...
def print(*args, **kwargs):
    ...
    kwargs2 = kwargs.copy()
    kwargs2["file"] = LOG_FILE

    builtins.print(*args, **kwargs2)
...
def run(config):
    ...
    # Keep compatibility with the existing checkpoint saving code
    bert_config = model_metadata.get("bert_config", None)
    ...
    if False:
        ...
        # Constant masking rate (configurable)
        mask_ratios = [config.mask_ratio for _ in range(config.epochs + 1)]
    ...
    if config.global_rank == 0:
        ...
        print("=" * 80)
        print(f"Training session started: {datetime.now()}")
        print("=" * 80)
        print()
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        if config.model_output_dir and (not config.distributed or config.global_rank == 0):
            safe_save_model(
                ...
                total_step=total_step,
                n_samples_seen=n_samples_seen,
                bert_config=bert_config.to_dict() if bert_config is not None else None,
                **best_stats,
            )
...
def train_one_epoch(
    ...
    for batch_idx, (sequences, y_true, att_mask) in enumerate(dataloader):
        ...
        if arch == "janus_convnova":
            # Janus-ConvNova does NOT use masked-token prediction.
            #
            # Every valid nucleotide is predicted from:
            #   left context  -> causal forward branch
            #   right context -> causal reverse branch
            #
            # The target nucleotide itself is excluded by the
            # directional architecture.
            special_tokens_mask = att_mask.bool()
            special_tokens_mask = special_tokens_mask.to(device)

            # No masking for Janus-ConvNova.
            masked_input = sequences.clone()

            # All valid positions contribute to the loss.
            input_maskout = special_tokens_mask.clone()

        else:

            # Existing Transformer / ConvNova masking behavior
            if arch in ["convnova", "convnova_long"]:
                # ConvNova-family models use the attention mask so only real
                # nucleotides are eligible for masking.
                special_tokens_mask = att_mask.bool()
            ...
            masked_input = sequences.clone()
            random_mask = torch.rand(masked_input.shape, device=device)

            input_maskout = random_mask < mask_ratio
            input_maskout &= special_tokens_mask

            if arch in ["convnova", "convnova_long"]:
                # ConvNova-family models: replace masked bases with N
                masked_input[input_maskout] = 4
            else:
        ...
        if arch == "transformer":
            ...
            targets = (
                sequences
                - n_special_tokens * (sequences > (n_special_tokens - 1))
            )

        elif arch in ["convnova", "convnova_long", "janus_convnova"]:

            # NucleotideTokenizer returns:
            # A=0 C=1 G=2 T=3 N=4
            targets = sequences
        ...
        if arch == "transformer":
            ...
            loss = criterion(
                out.logits.view(-1, 4**config.k_mer)[
        ...
        elif arch in ["convnova", "convnova_long"]:

            # ConvNova-family behavior:
            # loss ONLY on masked nucleotides.
            loss = criterion(
                out.logits.view(-1, 5)[
                    input_maskout.view(-1)
                ],
                targets.view(-1)[
                    input_maskout.view(-1)
                ],
            ...
            # Janus-ConvNova:
            # predict EVERY valid nucleotide.
            #
            # No masking is performed because the directional
            # architecture prevents the target nucleotide from
            # entering its own representation.
            loss = criterion(
                out.logits.view(-1, 5)[
                    special_tokens_mask.view(-1)
                ],
        ...
        with torch.no_grad():
            ...
            if arch == "janus_convnova":
                ...
                # Janus predicts every valid nucleotide.
                valid = special_tokens_mask

                acc_all = (
                    is_correct[valid].sum()
                    / valid.sum()
                )

                # For compatibility with the existing logging fields,
                # use the all-token accuracy as the main accuracy.
                acc_msk = acc_all
                acc_kpt = acc_all

            else:

                # Existing Transformer / ConvNova metrics
                acc_all = (
    ...
    with torch.no_grad():
        for batch_idx, (sequences, _y_true, att_mask) in enumerate(dataloader):
            ...
            if arch == "janus_convnova":
                ...
                # Janus-ConvNova is evaluated without masking.
                # Every valid nucleotide is predicted.
                special_tokens_mask = att_mask.bool().to(device)

                masked_input = sequences.clone()

                # Every valid nucleotide is part of the evaluation set.
                input_maskout = special_tokens_mask.clone()

            else:

                if arch in ["convnova", "convnova_long"]:
                    # Ignore padded positions during evaluation
                    special_tokens_mask = att_mask.bool()
            ...
            if arch == "transformer":
                ...
                # Janus-ConvNova:
                # loss over every valid nucleotide.
                loss = criterion(
                    out.logits.view(-1, 5)[
                        special_tokens_mask.view(-1)
                    ],
            ...
            if arch == "janus_convnova":
                ...
                # Every valid nucleotide is evaluated.
                valid = special_tokens_mask
                ...
                # Existing Transformer / ConvNova behavior
                if arch == "convnova":
                    valid = special_tokens_mask
                else:
                    # Ignore special tokens for Transformer
                    valid = targets > (n_special_tokens - 1)
                ...
                acc_msk = (
                    batch_size_this_gpu
                    * is_correct[input_maskout & valid].sum()
                    / (input_maskout & valid).sum()
                )
...
def get_parser():
    ...
    group.add_argument(
        ...
        type=float,
        default=0.10,
        help="Fraction of valid tokens to mask during MLM. Default: %(default)s",
    )
    parser.add_argument(
